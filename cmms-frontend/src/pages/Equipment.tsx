...
const EquipmentPage: React.FC<EquipmentPageProps> = ({ action, equipmentId }) => {
  ...
  const filteredEquipment = equipmentList.filter((equipment) => {
    const matchesSearch = equipment.manufacturerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      equipment.modelNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
      equipment.locationDescription.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ${selectedEquipment ? 'no-print' : ''}`}>
          {filteredEquipment.map((item) => (
            <motion.div
              ...
              className="bg-white rounded-lg shadow-md p-6 no-print"
            >
